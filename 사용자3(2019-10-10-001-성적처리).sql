-- 여기는 USER3 화면입니다.
 ...
 -- 학번은 테이블에 저장이 되어있는데 학번에 해당하는 학생이 누구인지 모르겠다.
 ...
 SELECT * FROM tbl_student SC, tbl_student ST
 WHERE SC.s_num = ST.st_num;
 
 -- 보고자 하는 칼럼만 나열해서 필요한 정보만 확인하기
 -- SELECT * 는 데이터가 많을 경우 조회하는데 다소 시간이 많이 소요
 ...
 -- 성적정보만 추가를 한 경우
 -- EQ JOIN을 실행시 새로운 학생정보의 성적은 출력이 되지 않는다.
 SELECT SC.s_num, ST.st_name, SC.s_kor, SC.s_eng, SC.s_math FROM tbl_score SC, tbl_student ST
 WHERE SC.s_num = ST.st_num;
 
 -- SCORE 테이블과 STUDENT 테이블간에 참조무결성이 유지되지 않고 있다.
 -- 참조무결성이 완전하지 않은 경우는 EQ JOIN으로 데이터를 조회하면 안된다.
 -- 이경우에는 실제 저장되어 있는 데이터들이 누락되어 출력되기 때문에
 -- LEFT JOIN을 사용해서 조회
 SELECT SC.s_num, ST.st_name, SC.s_kor, SC.s_eng, SC.s_math
    FROM tbl_score SC -- 조회하고자 하는 주(main) 데이터
        LEFT JOIN tbl_student ST -- 연결해서 보고자하는 보조데이터(sub, detail) 데이터
 ...
 -- 매우 복잡한 SQL문을 매번 실행하는 것은 매우 비효율적이다.
 -- DBMS에서는 SELECT SQL문을 마치 물리적 table인 것처럼 취급할 수 있도록
 -- VIEW OBJECT를 제공한다.
 -- VIEW OBJECT를 SELECT실행을 하면
 ...
 -- LIKE에서 %가 뒤에나오는 조회명령문은
 -- 가급적 사용을 자제(성능이 떨어진다(조금))
 SELECT * FROM view_score
 WHERE d_name LIKE '컴퓨터%';
 
 -- LIKE에서 %가 앞에 나오는 조회명령문은
 -- 가급적 사용을 자제(성능이 떨어진다(많이)
 SELECT * FROM view_score
 WHERE d_name LIKE '%공학';
 
 -- 값과 값을 연결하여 하나의 문자열처럼 출력
 /*
 SELECT s_num || ' : ' || st_name : 오라클 전용
 ...
 */
 ...
 -- Java등 프로그래밍 코딩으로 SQL을 작성할 때는
 -- LIKE 키워드의 문자열을 연결문자열로 작성해야 한다.
 SELECT * FROM view_score
 WHERE d_name LIKE '컴퓨터' || '%';
 
 -- 해당 칼럼의 데이터가 다수 존재할 때
 -- 중복되지 않는 데이터만 출력하는 키보드
 -- 같은 칼럼에 있는 동일한 값은 한번만 출력하라
 SELECT DISTINCT s_num,st_name
 FROM view_score
 ORDER BY s_num;
 
 -- 다수의 칼럼을 DISSTINCT로 묶으면
 -- 원하는 결과가 나오지 않는다.
 ...
 --     각 학과별로 그룹으로 묶고 국어점수으 ㅣ총점을 계산한 후
 --     학과 번호 순으로 보여달라
 --     부분합 집계
 ...
 ORDER BY s_dept;
 -- GROUP BY로 묶어서 부분합을 보고자 할 때
 -- 기준으로 하는 칼럼이외에 SELECT문에 나열된 칼럼들 중
 -- 집계함수로 감싸지 않은 칼럼들은

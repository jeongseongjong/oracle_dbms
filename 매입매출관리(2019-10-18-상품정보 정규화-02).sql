-- IOLIST 사용자 화면
 ...
 -- 판매가격 = ROUNG(판매가격 / 10,0) * 10
 -- * 1개이상의 (전체) 데이터를 대상으로 UPDATE, DELETE를 수행할 때는
 ...
 -- 서브쿼리를 사용해서
 -- UPDATE 실행에서 유의사항
 --     UPDATE를 수행하는 SUBQ는 SELECT Projection에는
 --     칼럼을 1개만 사용해야 한다.
 --     SUBQ에서 나타나는 레코드 수도 반드시 1개만 나타나야 한다.
 
 -- 매입매출테이블 리스트를 나열하고
 -- 각 요소의 상품이름을 SUBQ로 전달
 -- SUBQ에서는 상품테이블로부터 상품이름을 조회하여
 -- 일치하는 레코드가 1개 나타나면
 -- 해당레코드의 상품코드 칼럼의 값을
 -- 매입매출칼럼의 상품코드 칼럼에 UPDATE하라
 ...
 -- 누락된 데이터가 없는지 확인
 -- * 테이블 JOIN을 하면서 TABLE alias를 설정하지 않고
 --       JOIN조건등을 사용했다.
 --     JOIN되는 table들에  JOIN 조건으로 
 ...
 대량의 INSERT, UPDAT, DELETE를 수행한 후
 데이터 검증이 완료되면 가급적 COMMIT을 수행하고
 다음으로 진행하자
 
 (궁금하면 찾아봐라)
 오라클에서는
 COMMIT ROLLBACK도 시점을 지정해서
 어디까지 ROLLBACK을 할 것인지 지정할 수 있다.

...
VALUES ('979-001', '오라클 프로그래밍', '성능출판사', '서진수', 30000);
...
-- DROP, MODIFY 등을 수행하지 말자
 ...
 -- 현재 book 테이블에 추가된 데이터들에 장로칼럼이 비어있는 상태이다
 -- 장르칼럼의 데이터를 채워 넣기
 SELECT * FROM tbl_books;
 
 -- UPDATE를 실행할 때 2개 이상의 레코드에 영향을 미치도록
 -- 명령을 수행하는 것은 특별한 경우가 아니면 지양을 하자
 -- UPDATE 수행은 먼저 PK를 확인한 후 WHERE 절에 PK조건을 설정하여 하자
 UPDATE tbl_books
 SET b_genre = '데이터베이스'
 ...
 -- books 테이블의 데이터중에 경로가 장편소설인 데이터를 '장로소설' 로 바꾸고 싶다.
 UPDATE tbl_books
 SET b_genre = '장르소설'
 WHERE b_genre = '장편소설';
 ...
 -- 일부 데이터에 빈칸등이 잘못 삽입되어 데이터를 조회할 때
 -- 문제가 발생하는 것을 알게 되었다.
 -- 특히, 장르별로 데이터를 조회했더니 
 -- 장르 이름은 같은데 일부 데이터가 조회되지 않는 현상 발견
 -- 이러한 논리적 문제를 해결하기 위해 '장르 테이블'을 별도로 생성하고
 ...
 -- 도서정보를 확인하면서
 -- 장르칼럼의 코드값 대신 장르 이름으로 보고싶다.
 
 -- 테이블의 JOIN 수행
 -- 2개이상의 테이블을 서로 연계해서 하나의 리스트로 보여주는 것
 -- Relation ship
 ...
 -- 완전 Join, EQ Joing이라고 하며
 -- 결과를 카티션곱 이라고 한다.
 -- tabl1과 table2를 Relation 할때
 -- 서로 연결하는 칼럼의 값이 두 테이블에 모두 존재할 때
 -- 정상적인 결과를 낼 수 있다.

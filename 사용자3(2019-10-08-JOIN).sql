 ...
 -- 두테이블 간에 완전 참조무결성이 보장되는 경우
 -- 이 조인이 표시하는 리스트를 카티션곱 이라고 표현
 SELECT * FROM tbl_books B, tbl_genre G
 WHERE B.b_genre = G.g_code;
 
 -- EQ 조인의 경우
 -- 두 테이블이 완전 참조 무결성 조건에 위배되는 경우
 
 /*
 참조 무결성
 
 원본 Table(칼럼값)  =         참조Table(칼럼값)
 ------------------------------------------------
 값이 있다          >>         반드시 있다.
 있을 수도 있다.    <<         있다.
 절대 없다          <<         없다.
 ...
 */
 ...
 -- 그래서 새로운 장르코드를 생성해서 010이라고 사용하기로 결정
 ...
 -- 장르가 '장편소설'인 도서정보를 '장르소설'로 명칭을 변경하고 싶다.
 ...
 -- 만약 tbl_books 테이블에 장르칼럼을 일므으로 설정했더라면
 ...
 -- 즉, UPDATE, DELETE 등을 수행할 때는 1개의 레코드만 변경되도록
 -- PK를 WHERE 조건에 설정해서 명령문을 수행하는 것이 좋다.
 
 -- 이러한 이유로 테이블을 분리하고, 두 테이블을 JOIN해서 리스트를 출력하는
 -- 복잡한 과정을 수행해야 한다.
 
 -- 보통 1개의 테이블에 존재하는 데이터를 다수(2개)의 테이블로 분리하고
 -- JOIN을 수행할 수 있도록 구조를 변경하는 작업을 데이터베이스 '제2 정규화(2nd NF)라고 한다.

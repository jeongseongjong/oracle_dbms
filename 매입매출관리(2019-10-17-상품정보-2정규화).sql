...
-- 매입매출정보에 상품정보가 어떻게 저장되어있는지 확인
 ...
 -- 검증을 하기 위해 tbl_iolist와 tbl_product를 EQ JOING해서 조회
 SELECT io_inout, COUNT(*)
 FROM tbl_iolist IO, tbl_product P
 WHERE IO.io_pname = P.p_name 
 GROUP BY io_inout ;
 
 -- 상품정보 테이블의 단가를 어떻게 세팅을 할것인가 곰인곰인
 ...
 WHERE IO.io_pname = P.p_name 
    AND IO.io_inout = 1
 GROUP BY io_inout, IO.io_pname, io_price;
 
 
 -- 매입매출 테이블에서 매입단가를 조회해 봤더니
 -- 다행히 상품이름이 같은데 단가가 다른 상품이 없는것 같다.
 ...
 -- 상품거래정보에서 상품정보 매입, 매출단가를 생성을 했더니
 -- NULL인 값이 있다.
 -- 어떤상품은 매입 또는 매출만 된 경우
 
 /*
  매입단가에서 매출단가 생성하기
  공산품일경우 맹비단가의 약 18%를 더해서 매출단가 계산
  그리고 10%의 VAT를 붙여서 다시

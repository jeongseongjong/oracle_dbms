 -- 여기는 GRADE 화면
 -- PK지정방법을 칼럼에 primary key를 지정하지 않고
 -- 별도로 CONSTRAINT 추가 방식으로 지정
 -- 표준 SQL에서는 PK 지정방식을 
 -- 칼럼에 PRIMARY KEY 키워드 지정방식으로 사용하는데
 -- 간혹 표준 SQL의 PK 지정방식이 안되는 DBMS가 있다.
 -- 이때는 별도의 CONSTRAINT 추가 방식으로 지정해야 한다.
 -- 이 방식은 다수의 칼럼으로 PK를 지정할 때도 사용한다.
...
/*
 성적테이블을 생성할 때
 각 과목이름으로 칼럼을 만들어서 생성
 데이터를 추가하거나, 단순 조회를 할 때는 상당히 편리하게 사용할 수 있다.
 하지만, 사용중에 과목이 추가되거나 과목명이 변경되는 경우
 테이블의 칼럼을 변경해야 하는 상황이 발생
    테이블의 칼럼을 변경하는 것은 DBMS입장에서나 사용자 입장에서
    많은 비용(사용중지 시간, 노력, 위험성 등등)을 지불해야 한다.
    테이블의 칼럼을 변경하는 일은 매우 신중해야 한다.
 
 그래서 실제데이터는 고정된 칼럼으로 생성된 테이블에 저장을 하고
 View로 확인을 할 때 PIVOT 방식으로 펼쳐보면
 마치 실제 테이블에 칼럼이 존재하는 것처럼 사용 할 수 있는 장점이 있다.
*/
 ...
 FROM tbl_score
 WHERE s_std = '갈한수' GROUP BY s_std
 ORDER BY s_std;
 
 -- 오라클 11버전 이후 사용가능한 PIVOT 전용문법

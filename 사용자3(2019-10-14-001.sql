 ...
 -- 간단한 연산을 SELECT 문을 이용해서 실행할 수 있도록 하고있다.
 -- SELECT 30 + 40
 -- 오라클에서는 SELECT 다음에 FROM절이 없으면 문법오류를 발생하도록 설계
 -- 그래서 표준 SQL에서 사용하는 간단한 코드를 실행하기가 매우 까다롭다.
 -- 그래서 문법오류를 방지하고 간단한 코드를 실행하도록 하기 위해
 -- DUAL이라는 Dummy Table을 준비해 두었다.
 SELECT * FROM DUAL;
 ...
 -- 반복되어서 실행된다.
 -- 그래서 1개의 레코드만 가진 Dummy TABLE을 준비해두고
 -- 이 TABLE을 활용하여 코드를 실행하도록 하는것
 SELECT '1' AS num, '홍길동' AS name, '컴공과' AS dept FROM tbl_student;
 
 -- 가상의 데이터 하나 생성
 -- UNION
 -- 여러 테이블을 SELECT해서 생성된 view 결과를
 -- 묶어서 마치 하나의 결과처럼 보고자 할 때 사용
 SELECT '1' AS num, '홍길동' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '이몽룡' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '성춘향' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '장보고' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '임꺽정' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '장영실' AS name, '컴공과' AS dept FROM dual
 UNION ALL SELECT '1' AS num, '장길산' AS name, '컴공과' AS dept FROM dual;
 
 -- 실제 사용하는 DBMS에 물리적인 테이블을 생성하지 않고
 -- 가상의 데이터를 만들어서 사용하고자 할때 임시로 TABLE구조와 데이터를
 -- 생성하여 테스트 용도로 사용하는 명령 군
 -- 이 명령군을 사용할 때 UNIO 키워드를 사용한다.
 ...
 UNION ALL SELECT '총점' AS 학번, to_char(SUM(s_kor),'99,999') AS 국어,
                       ...
                       to_char(SUM(s_math),'99,999') AS 수학 FROM tbl_score group by '총점'
 UNION ALL SELECT '=====' AS 학번,'====' AS 국어,'====' AS 영어,'====' AS 수학 FROM DUAL;

 -- to_char(값, 형식)
 -- 숫자형 자료를 문자열형 자료로 변환시키는 casading 함수
 -- 9 : 숫자자릿수를 나타내는 형식,
 --     실제 출력되는 값의 자릿수만큼 개수를 지정해야 한다.
 --     to_char(123, '99999') >> 123
 -- 0 : 숫자자릿수를 나타내는 형식, 
 --     실제 출력되는 값보다 자릿수가 많으면 나머지는 0으로 채움
 ...
 -- UNION
 -- 테이블의 결과를 결합하여 하나의 VIEW처럼 보여주는 형식
 /* ===============================
        UNION            UNION ALL
------------------------------------
        중복데이터       무조건결합
...
-- 임시로 사용하는 TABLE 생성
-- WITH AS
-- SQL 테스트등을 위해 임시로 TABLE을 생성하는 오라클 SQL
 WITH tbl_temp AS 
(
...
(
 ...
 /*
    WHERE절에 포함된 SELECT 실행 : SELECT FROM tbl_temp
    '기은성', '남동예', 내세원', '방채호', '배채호'리스트를 생성하고
    WHERE IN ('기은성', '남동예', 내세원', '방채호', '배채호')코드를 생성한다.
 ...
 */
 ...
 -- 2. FROM절에 포함는 SUBQ
 --    인라인뷰라고 하며 다른 TABLE의 결과를 FROM 절에서 사용하는 것
 ...
 -- 3. SELECT SUB
 --     스칼라서브 쿼리
 --     1, 2쿼리와 달리 SUBQ에서는 절대 LIST를 출력하면 안된다.
 ...
 -- 다. SUBQ에서 WHERE절을 실행하여 데이터 추출
 -- 라. SUB(s_kor+s_eng+s_math)를 실행하여
 -- 마. 결과를 ruturn
 -- 바. main에서 st_num, 리턴 받은결과 형식으로 표시
 SELECT st_num,
 (

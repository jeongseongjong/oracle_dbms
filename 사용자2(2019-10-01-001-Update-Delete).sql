 ...
 -- 현재 Transaction(전체처리)가 완료 되었다 라는 것을
 -- DBMS에게 알리는 명령
 -- DCL(TCL)
 COMMIT;
 ...
 -- 이전에 COMMIT을 수행한 이후
 -- 데이터의 추가, 수정, 삭제를 수행한 것을 취소하는 명령
 ROLLBACK;
 SELECT * FROM tbl_address;
 
 -- UPDATE 명령을 기본형으로 수행을 하게되면
 -- 모든 Record 데이터가 변경이 되어버리는 사태가 발생
 -- 절대 UPDATE 명령은 기본형으로만 수행하지말기 바랍니다.
 
 -- tbl_address table에 저장된 데이터들 중에서
 -- name칼럼의 값이 '홍길동'인 데이터를 찾아서
 ...
 -- 이몽룡과 성춘향의 주소를 잘못변경한것이 발견됨
 -- 이몽룡의 주소를 '남원시'에서 '익산시'로 변경
 ...
 --         도 한개는 광주광역시로 바꾸고 싶을때
 --         실행할 수 있는 방법이 없다.
 -- 결론적으로 이 table은 설계상 miss로 인해
 -- '개체무결성'이 훼손되었다 라고 할 수 있다.
 ...
 -- 주소를 추가하고 보니
 -- 전화번호 칼럼에 주소를 입력하고
 -- 더불어 서울특별시에 사는 홍길동의 데이터가 3개 중복 추가된것을 확인
 -- 원본 데이터를 봤더니
 -- 홍길동이 동명이인으로 서울특별시, 광주광역시, 부산광역시에 거주하는것으로 확인
 -- 한개의 홍길동 데이터는 그대로 두고
 -- 한개는 주소를 광주광역시로 나머지한개는 주소를 부산광역시로 바꾸려 한다.
...
-- DELETE명령도 UPDATE와 마찬가지로 기본형으로 실행하지말기; 
 ...
 -- DELETE를 실행할때도 table에 PK 가 있으면 반드시 PK단위로 데이터를 삭제하자
 -- 1. 삭제하고 하는 데이터 있는지 여러방법으로 조회
 SELECT * FROM tbl_address WHERE name = '성춘향'; -- 성춘향 데이터는 1개뿐이므로
 DELETE FROM tbl_address WHERE name = '성춘향'; -- 이라고 실행해도 되지만
 
 SELECT * FROM tbl_address WHERE name = '홍길동'; -- 홍길동 데이터는 3개가 출력되어
 DELETE FROM tbl_address WHERE name = '홍길동'; -- 이라고 실행하면 절대안됨
 
 SELECT * FROM tbl_address WHERE name = '홍길동' AND address = '서울특별시';
 DELETE FROM tbl_address WHERE name = '홍길동' AND address = '서울특별시' ; -- 이라고 해도 되지만
 -- 이명령도 개체무결성을 보장하는 데에는 문제가 발생할 소지가 내포한다.
 ...
 -- 번거롭고 불편하지만 PK칼럼을 WHERE 조건으로 하여
 -- 명령을 수행하자
 
 -- DBMS를 운영하는 과정에서
 -- 만에하나 재난(실수로 수행명령으로 데이터 변경, 삭제, 천재지변)이 발생했을 때
 -- 데이터를 복구할 수 있는 준비를 해야한다.
 -- 1. 백업 : 업무가 종료된 후 데이터를 다른 저장소, 저장매체에 복사하여 보관
 -- 1-1. 복구 : 백업해둔 데이터를 사용중인 시스템에 다시 설치하여 사용할 수 있도록 하는것
 --         복구는 원상태로 만드는데 상당한 시간이 필요하고
 --         백업된 시점에 따라 완전복구가 되지 않는 경우도 있다.
 -- 2. 로그기록(저널링 복구) : INSERT, UPDATE, DELETE 명령이 수행될 때
 --        수행되는 모든 명령들을 별도의 파일로 기록해 두고
 --         문제가 발생했을 때 로그를 다시 역으로 추적하여 복구하는 방법
 -- 3. 이중화, 상품화
 --     실제 운영중인 운영체제, DBMS, storage등을 똑같은 구조로
 --     설치 위치를 달리하여 동시에 운영하는것
 --     재난이 발생하면 발생 지역의 시스템을 단절하고
 --     정상 시스템으로 절환하여 운영을 계속하도록 하는 시스템
 
 -- 데이터센터(데이터웨어 하우스)
 --         대량의 데이터베이스를 운영하는 서버시스템을 모아서
 --         통합 관리하는 곳

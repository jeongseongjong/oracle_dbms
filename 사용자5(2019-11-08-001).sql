...
/*
 ...
 객체 무결성 보장을 위하여 PK를 설정하는데
 가장 좋은 방법은 1개의 유일한 값을 갖는 칼럼을 선정하여 PK로 설정
 하지만 그렇지 못한경우 2개이상의 칼럼을 묶어서 PK로 설정하기도 한다.
...
*/
 ...
 1. 전체 칼럼에 모든데이터를 다 입력하고 데이터를 추가할 때만 가능
 2. table의 칼럼 순서대로 데이터를 나열해야 한다.
    만약 업무(프로젝트)가 진행되는 과정에서 table의 칼럼순서가 변경되는 경우,
    칼럼을 추가하는경우 보통 table구조에서 끝부분에 칼럼이 추가되는데
    이때는 데이터가 순서대로 원하는 칼럼에 저장될 것이다. 라는 보장을 할 수 없게 된다.
 */

 -- m_seq 칼럼의 값이 1인데이터가 추가된 상태에서
 -- 다시 m_seq칼럼의 값이 1인 데이터를 또 추가하려고 하면 
 -- PK규칙 위배되서 데이터가 추가되지 않는다.
 -- 이런 상황에서 새로운 데이터를 추가할 때마다
 -- m_seq 값을 기존값과 중복되지 않은 값으로 찾아야하는데
 -- 이 방법이 업무진행에서 번거로운 작업이 된다.
 INSERT INTO tbl_memo(m_seq,m_date,m_time,m_auth,m_subject)
 VALUES(1,'2019-11-08','09:42:00','홍길동','메모작성');
 ...
 -- 데이터를 입력해놓고 봤더니
 -- m_seq가 15인 데이터는 잘못한것이 발견되었다.
 -- m_seq가 15인데이터는 필요가 없는데이터이다
 -- 데이터를 삭제하자.
...
p_vat   	VARCHAR2(1)
 ...
 -- 매입 매출의 합계를 월별로 부분합을 구분하여 PIVOT방식으로 보기
   SELECT
...
SELECT
 ...
 -- EQ조인(inner 조인)은 JOIN하는 테이블이 서로 참조무결성이 보장되는 경우 가능
 -- 참조 무결성이 위배되는 테이블을 EQ JOIN을 실행하면
 -- 위배된 데이터는 나오지 않는다.
 SELECT * 

...
S_SUBJECT          VARCHAR2(4)   
 ...
 학생정보 칼럼을 제2정규화
 score 테이블에는 학생이름이 일반적인 문자열로 저장이 되어있다.
 일반적인 문자열로 저장된 경우
 학생이름을 변경해야 할 경우가 발생하면 
 UPDATE tbl_score
 SET s_std = '이몽룡'
 WHERE s_std = '이멍룡'
 위와 같은 방식으로 칼럼 UPDATE 명령을 수행할 것이다.
 하지만 DBMS의 UPDATE 권장 패턴에서는
 여러개의 레코드를 수정, 변경하는 것을 지양하도록 한다.
 
 또한 만약 학생정보에 다른정보를 포함하고 싶을때는
 tbl_score 테이블에 칼럼을 추가하는 등의 방식으로
 처리를 해야 하는데
 이 방식 또 한 좋은 방식이 아니다.
 
 그래서 학생정보 테이블을 생성하고 학생코드를 만들어 준 다음
 score 테이블의 s_std칼럼의 값을 학생코드로 변경하여
 제2 정규화를 수행하고자 한다.
 ...
 */
 ...
 ALTER TABLE tbl_dept RENAME COLUMN d_dept TO d_name;
 ...
 */
 ...
 -- 제2 정규화가 완료된 4개의 TABLE을 서로 relation 설정
 -- 참조무결성 제약조건 설정

 ...
 INSERT INTO tbl_student(st_num, st_name, st_dep_no, st_grade)
 VALUES('A0002', '이몽룡','002', 2);
 
 INSERT INTO tbl_student(st_num, st_name, st_dep_no, st_grade)
 VALUES('A0003', '성춘향','003', 3);
 
 INSERT INTO tbl_student(st_num, st_name, st_dep_no, st_grade)
 VALUES('A0004', '임꺽정','004', 2);
 
 INSERT INTO tbl_student(st_num, st_name, st_dep_no, st_grade)
 VALUES('A0005', '장보고','005', 3);
 ...
 -- tbl_student의 테이블에 저장된 데이터 레코드가 모두 몇개인가
 SELECT COUNT(*)
 FROM tbl_student;
 ...
 -- 오라클 이외에 다른 DBMS에서는
 -- 간단한 연산등을 수행할 때
 -- SELECT 연산 형식으로 사용된다.(FROM 이후 없이 사용가능)
 SELECT 30 + 40 FROM dual;
 SELECT * FROM dual;
 
 -- 레코드 셋(Record Set) : SELECT 명령이 실행된 후 보여주는 리스트를
 SELECT 30 * 40 FROM tbl_student;
 
 SELECT * FROM tbl_student;
 
 --학생들의 주소칼럼의 데이터가 생략된(null) 상태
 -- 성춘향 주소 광주
 -- 이몽룡 주소 익산
 -- 홍길동 주소 서울
 -- 이미 존재하는 레코드의 특정칼럼에 값을 입력하고자 할때는
 -- UPDATE 명령을 수행한다.
 ...
 -- 그래서 이번에는 이름과 학년을 WHERE  조건에 걸고 UPDATE를 수행하려한다.
 -- 또, 2. 이름이 같고 학년이 같은 학생들이 다수 보인다.
 -- 결국 이름, 학년, 학과까지 WHERE 조건에 걸어 UPDATE 수행하려고 하는데
 -- 3칼럼의 값이 같은 학생들이 다수 있따.
 -- 이제 지금까지 WHERE 조건에 설정한 항목들은 무의미 해져버렸다.
 
 -- 결론
 -- UPDATE를 수행하고자 할때는
 -- 학생테이블의 PK로 설정된 학생의 학번(st_num)을 알고
 -- 학번을 기준으로 UPDATE를 수행해야만 한다.
 -- -> 데이터의 신뢰도 유지 = 개체무결성 보장이라고 한다.
